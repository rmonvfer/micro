...
/** AI Gateway to OpenAI passthrough. Used until /compat supports /v1/responses. */
const CLOUDFLARE_AI_GATEWAY_OPENAI_BASE_URL =
	"https://gateway.ai.cloudflare.com/v1/{CLOUDFLARE_ACCOUNT_ID}/{CLOUDFLARE_GATEWAY_ID}/openai";

/** AI Gateway to Anthropic passthrough. */
...
async function generateModels() {
	...
	// micro speaks four wire protocols. A provider whose models need another one is left
	// out rather than shipped as a listing that cannot be used.
	...
	const DESCRIBED: { id: string; name: string; env: string[]; oauth?: true; label?: string }[] = [
		...
		{ id: "github-copilot", name: "GitHub Copilot", env: ["COPILOT_GITHUB_TOKEN"], oauth: true, label: "GitHub Copilot token" },
		{ id: "openrouter", name: "OpenRouter", env: ["OPENROUTER_API_KEY"], label: "OpenRouter API key" },
		{ id: "ant-ling", name: "Ant Ling", env: ["ANT_LING_API_KEY"], label: "Ant Ling API key" },
		{ id: "cerebras", name: "Cerebras", env: ["CEREBRAS_API_KEY"], label: "Cerebras API key" },
		...
		{ id: "deepseek", name: "DeepSeek", env: ["DEEPSEEK_API_KEY"], label: "DeepSeek API key" },
		{ id: "fireworks", name: "Fireworks", env: ["FIREWORKS_API_KEY"], label: "Fireworks API key" },
		{ id: "groq", name: "Groq", env: ["GROQ_API_KEY"], label: "Groq API key" },
		{ id: "huggingface", name: "Hugging Face", env: ["HF_TOKEN"], label: "Hugging Face token" },
		{ id: "kimi-coding", name: "Kimi For Coding", env: ["KIMI_API_KEY"], label: "Kimi API key" },
		{ id: "minimax", name: "MiniMax", env: ["MINIMAX_API_KEY"], label: "MiniMax API key" },
		{ id: "minimax-cn", name: "MiniMax CN", env: ["MINIMAX_CN_API_KEY"], label: "MiniMax CN API key" },
		{ id: "moonshotai", name: "Moonshot AI", env: ["MOONSHOT_API_KEY"], label: "Moonshot AI API key" },
		{ id: "moonshotai-cn", name: "Moonshot AI CN", env: ["MOONSHOT_API_KEY"], label: "Moonshot AI API key" },
		{ id: "nvidia", name: "NVIDIA", env: ["NVIDIA_API_KEY"], label: "NVIDIA API key" },
		{ id: "opencode", name: "OpenCode Zen", env: ["OPENCODE_API_KEY"], label: "OpenCode API key" },
		{ id: "opencode-go", name: "OpenCode Zen Go", env: ["OPENCODE_API_KEY"], label: "OpenCode API key" },
		{ id: "together", name: "Together", env: ["TOGETHER_API_KEY"], label: "Together API key" },
		{ id: "vercel-ai-gateway", name: "Vercel AI Gateway", env: ["AI_GATEWAY_API_KEY"], label: "Vercel AI Gateway API key" },
		{ id: "xai", name: "xAI", env: ["XAI_API_KEY"], label: "xAI API key" },
		{ id: "xiaomi", name: "Xiaomi", env: ["XIAOMI_API_KEY"], label: "Xiaomi API key" },
		{ id: "xiaomi-token-plan-ams", name: "Xiaomi Token Plan AMS", env: ["XIAOMI_TOKEN_PLAN_AMS_API_KEY"], label: "Xiaomi Token Plan AMS API key" },
		{ id: "xiaomi-token-plan-cn", name: "Xiaomi Token Plan CN", env: ["XIAOMI_TOKEN_PLAN_CN_API_KEY"], label: "Xiaomi Token Plan CN API key" },
		{ id: "xiaomi-token-plan-sgp", name: "Xiaomi Token Plan SGP", env: ["XIAOMI_TOKEN_PLAN_SGP_API_KEY"], label: "Xiaomi Token Plan SGP API key" },
		{ id: "zai", name: "Z.AI", env: ["ZAI_API_KEY"], label: "Z.AI API key" },
		{ id: "zai-coding-cn", name: "Z.AI Coding CN", env: ["ZAI_CODING_CN_API_KEY"], label: "Z.AI Coding CN API key" },
	];

	/** The value most of a provider's models agree on, which becomes the provider's own. */
	function commonest<T>(values: (T | undefined)[]): T | undefined {
	...
	for (const described of DESCRIBED) {
		const offered = Object.values(providers[described.id] ?? {});
		if (offered.length === 0) {
			omitted.push(`${described.id} (no models)`);
			continue;
		}
		if (!offered.every((model) => SPOKEN[model.api])) {
			omitted.push(`${described.id} (${commonest(offered.map((model) => model.api))})`);
			continue;
		}

		const api = commonest(offered.map((model) => SPOKEN[model.api]));
		const baseUrl = commonest(offered.map((model) => model.baseUrl));
		...
		const models = offered
			...
			.map((model) => {
				...
				if (model.baseUrl && model.baseUrl !== baseUrl) entry.base_url = model.baseUrl;
				if (model.headers) entry.headers = model.headers;
				if (model.compat) entry.compat = model.compat;
				if (model.thinkingLevelMap) entry.thinking = model.thinkingLevelMap;
				return entry;
			});
	...
	writeFileSync(
		join(repoRoot, "crates/micro-models/data/catalog.json"),
		`${JSON.stringify({ providers: catalog }, undefined, "\t")}\n`,
	);
	writeFileSync(
		join(repoRoot, "crates/micro-auth/data/providers.json"),
		`${JSON.stringify(table, undefined, "\t")}\n`,
	);
	for (const id of Object.keys(providers)) {
		if (!DESCRIBED.some((described) => described.id === id)) {
			omitted.push(`${id} (${commonest(Object.values(providers[id]).map((model) => model.api))})`);
		}
	}

	console.log(`Wrote ${table.length} providers to crates/micro-models/data/catalog.json`);
	if (omitted.length > 0) console.log(`Left out: ${omitted.sort().join(", ")}`);
