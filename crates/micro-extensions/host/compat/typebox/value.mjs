...
function CheckAdditionalItems(stack, context, schema2, value) {
  ...
  const isAdditionalItems = value.every((item, index) => {
    return exports_guard.IsLessThan(index, schema2.items.length) || CheckSchemaPushStack(stack, context, schema2.additionalItems, item) && context.AddIndex(index);
  });
...
function ErrorAdditionalItems(stack, context, schemaPath, instancePath, schema2, value) {
  ...
  const isAdditionalItems = value.every((item, index) => {
    ...
    return exports_guard.IsLessThan(index, schema2.items.length) || ErrorSchemaPushStack(stack, context, nextSchemaPath, nextInstancePath, schema2.additionalItems, item) && context.AddIndex(index);
  });
  return isAdditionalItems;
}
...
function ErrorAllOf(stack, context, schemaPath, instancePath, schema2, value) {
  ...
  const results = schema2.allOf.reduce((result, schema3, index) => {
...
function ErrorAnyOf(stack, context, schemaPath, instancePath, schema2, value) {
  const failedContexts = [];
  const results = schema2.anyOf.reduce((result, schema3, index) => {
    const nextContext = new AccumulatedErrorContext;
...
function CheckItemsSized(stack, context, schema2, value) {
  return exports_guard.Every(schema2.items, 0, (schema3, index) => {
    return exports_guard.IsLessEqualThan(value.length, index) || CheckSchemaPushStack(stack, context, schema3, value[index]) && context.AddIndex(index);
  });
}
function ErrorItemsSized(stack, context, schemaPath, instancePath, schema2, value) {
  return exports_guard.EveryAll(schema2.items, 0, (schema3, index) => {
    const nextSchemaPath = `${schemaPath}/items/${index}`;
    const nextInstancePath = `${instancePath}/${index}`;
    return exports_guard.IsLessEqualThan(value.length, index) || ErrorSchemaPushStack(stack, context, nextSchemaPath, nextInstancePath, schema3, value[index]) && context.AddIndex(index);
  });
}
function CheckItemsUnsized(stack, context, schema2, value) {
  ...
  return exports_guard.Every(value, offset, (element, index) => {
    return CheckSchemaPushStack(stack, context, schema2.items, element) && context.AddIndex(index);
  });
}
function ErrorItemsUnsized(stack, context, schemaPath, instancePath, schema2, value) {
  ...
  return exports_guard.EveryAll(value, offset, (element, index) => {
    ...
    const nextInstancePath = `${instancePath}/${index}`;
    return ErrorSchemaPushStack(stack, context, nextSchemaPath, nextInstancePath, schema2.items, element) && context.AddIndex(index);
  });
}
...
function ErrorOneOf(stack, context, schemaPath, instancePath, schema2, value) {
  ...
  const passedContexts = schema2.oneOf.reduce((result, schema3, index) => {
    const nextContext = new AccumulatedErrorContext;
...
function CheckPrefixItems(stack, context, schema2, value) {
  return exports_guard.IsEqual(value.length, 0) || exports_guard.Every(schema2.prefixItems, 0, (schema3, index) => {
    return exports_guard.IsLessEqualThan(value.length, index) || CheckSchemaPushStack(stack, context, schema3, value[index]) && context.AddIndex(index);
  });
}
function ErrorPrefixItems(stack, context, schemaPath, instancePath, schema2, value) {
  return exports_guard.IsEqual(value.length, 0) || exports_guard.EveryAll(schema2.prefixItems, 0, (schema3, index) => {
...
function CheckUnevaluatedItems(stack, context, schema2, value) {
  ...
  return exports_guard.Every(value, 0, (item, index) => {
    return (indices.has(index) || CheckSchema(stack, context, schema2.unevaluatedItems, item)) && context.AddIndex(index);
  });
}
function ErrorUnevaluatedItems(stack, context, schemaPath, instancePath, schema2, value) {
  ...
  const isUnevaluatedItems = exports_guard.EveryAll(value, 0, (item, index) => {
    const nextContext = new AccumulatedErrorContext;
    const isEvaluatedItem = (indices.has(index) || ErrorSchema(stack, nextContext, schemaPath, instancePath, schema2.unevaluatedItems, item)) && context.AddIndex(index);
    if (!isEvaluatedItem)
...
function Assert(...args) {
  ...
  const check3 = Check2(context, type3, value);
  if (!check3)
    throw new AssertError("Assert", value, Errors2(context, type3, value));
}
...
// ../../../../../../../.micro/npm/node_modules/typebox/build/type/types/_refine.mjs
...
function IsRefine2(value) {
  return IsSchema2(value) && exports_guard.HasPropertyKey(value, "~refine") && exports_guard.IsArray(value["~refine"]) && exports_guard.Every(value["~refine"], 0, (value2) => IsRefinement(value2));
}
...
function GetProperties(type3) {
  const result = IsObject2(type3) ? type3.properties : IsTuple(type3) ? TupleElementsToProperties(type3.items) : Unreachable();
  return result;
}
...
function IsObjectLike(type3) {
  return IsObject2(type3) || IsTuple(type3);
}
...
function StringOrNumberCheck(types2) {
  return types2.some((type3) => IsString3(type3) || IsNumber3(type3) || IsInteger2(type3));
}
...
function EncodeType(type3, right, pattern3) {
  return IsEnum2(type3) ? EncodeEnum(type3.enum, right, pattern3) : IsInteger2(type3) ? EncodeInteger(right, pattern3) : IsLiteral(type3) ? EncodeLiteral(type3.const, right, pattern3) : IsBigInt2(type3) ? EncodeBigInt(right, pattern3) : IsBoolean3(type3) ? EncodeBoolean(right, pattern3) : IsNumber3(type3) ? EncodeNumber(right, pattern3) : IsString3(type3) ? EncodeString(right, pattern3) : IsTemplateLiteral(type3) ? EncodeTemplateLiteral(type3.pattern, right, pattern3) : IsTemplateLiteralDeferred(type3) ? EncodeTemplateLiteralDeferred(type3.parameters[0], right, pattern3) : IsUnion(type3) ? EncodeUnion(type3.anyOf, right, pattern3) : NeverPattern;
}
...
function TryRestInferable(type3) {
  return IsRest(type3) ? IsInfer(type3.items) ? IsArray2(type3.items.extends) ? Inferrable(type3.items.name, type3.items.extends.items) : IsUnknown(type3.items.extends) ? Inferrable(type3.items.name, type3.items.extends) : undefined : Unreachable() : undefined;
}
...
function FromType3(stack, context, type3) {
  return IsRef2(type3) ? FromRef(stack, context, type3.$ref) : IsArray2(type3) ? FromType3(stack, context, type3.items) : IsAsyncIterator2(type3) ? FromType3(stack, context, type3.iteratorItems) : IsConstructor2(type3) ? FromTypes2(stack, context, [...type3.parameters, type3.instanceType]) : IsFunction2(type3) ? FromTypes2(stack, context, [...type3.parameters, type3.returnType]) : IsInterfaceDeferred(type3) ? FromProperties(stack, context, type3.parameters[1]) : IsIntersect(type3) ? FromTypes2(stack, context, type3.allOf) : IsIterator2(type3) ? FromType3(stack, context, type3.iteratorItems) : IsObject2(type3) ? FromProperties(stack, context, type3.properties) : IsPromise(type3) ? FromType3(stack, context, type3.item) : IsUnion(type3) ? FromTypes2(stack, context, type3.anyOf) : IsTuple(type3) ? FromTypes2(stack, context, type3.items) : IsRecord(type3) ? FromType3(stack, context, RecordValue(type3)) : false;
}
...
function FromType4(context, type3, result) {
  return IsRef2(type3) ? FromRef2(context, type3.$ref, result) : IsArray2(type3) ? FromType4(context, type3.items, result) : IsAsyncIterator2(type3) ? FromType4(context, type3.iteratorItems, result) : IsConstructor2(type3) ? FromTypes3(context, [...type3.parameters, type3.instanceType], result) : IsFunction2(type3) ? FromTypes3(context, [...type3.parameters, type3.returnType], result) : IsInterfaceDeferred(type3) ? FromProperties2(context, type3.parameters[1], result) : IsIntersect(type3) ? FromTypes3(context, type3.allOf, result) : IsIterator2(type3) ? FromType4(context, type3.iteratorItems, result) : IsObject2(type3) ? FromProperties2(context, type3.properties, result) : IsPromise(type3) ? FromType4(context, type3.item, result) : IsUnion(type3) ? FromTypes3(context, type3.anyOf, result) : IsTuple(type3) ? FromTypes3(context, type3.items, result) : IsRecord(type3) ? FromType4(context, RecordValue(type3), result) : result;
}
...
function FromType5(type3) {
  return IsRef2(type3) ? FromRef3(type3.$ref) : IsArray2(type3) ? _Array_(FromType5(type3.items), ArrayOptions(type3)) : IsAsyncIterator2(type3) ? AsyncIterator(FromType5(type3.iteratorItems)) : IsConstructor2(type3) ? Constructor(FromTypes4(type3.parameters), FromType5(type3.instanceType)) : IsFunction2(type3) ? _Function_(FromTypes4(type3.parameters), FromType5(type3.returnType)) : IsIntersect(type3) ? Intersect(FromTypes4(type3.allOf)) : IsIterator2(type3) ? Iterator(FromType5(type3.iteratorItems)) : IsObject2(type3) ? _Object_(FromProperties3(type3.properties)) : IsPromise(type3) ? _Promise_(FromType5(type3.item)) : IsRecord(type3) ? Record(RecordKey(type3), FromType5(RecordValue(type3))) : IsUnion(type3) ? Union(FromTypes4(type3.anyOf)) : IsTuple(type3) ? Tuple(FromTypes4(type3.items)) : type3;
}
...
function EvaluateInstantiate(context, state, type3, options) {
  const instantiatedType = InstantiateType(context, state, type3);
...
function BindArgument(context, state, name, extends_, type3) {
  const instantiatedArgument = InstantiateType(context, state, type3);
...
function CallInstantiate(context, state, target2, arguments_) {
  ...
  const result = IsGeneric(type3) ? IsTailCall(state, name) ? CallConstruct(Ref2(name), instantiatedArguments) : CallImmediate(context, state, Ref2(name), type3.parameters, type3.expression, instantiatedArguments) : CallConstruct(target2, instantiatedArguments);
  return result;
}
...
function CapitalizeInstantiate(context, state, type3, options) {
  const instantiatedType = InstantiateType(context, state, type3);
  return CapitalizeAction(instantiatedType, options);
}
function LowercaseInstantiate(context, state, type3, options) {
  const instantiatedType = InstantiateType(context, state, type3);
  return LowercaseAction(instantiatedType, options);
}
function UncapitalizeInstantiate(context, state, type3, options) {
  const instantiatedType = InstantiateType(context, state, type3);
  return UncapitalizeAction(instantiatedType, options);
}
function UppercaseInstantiate(context, state, type3, options) {
  const instantiatedType = InstantiateType(context, state, type3);
...
function ConstructorParametersOperation(type3) {
  const parameters = IsConstructor2(type3) ? type3["parameters"] : [];
...
function ConstructorParametersInstantiate(context, state, type3, options) {
  const instantiatedType = InstantiateType(context, state, type3);
...
function ExcludeTypeLeft(left, right) {
  const check4 = Extends({}, left, right);
...
function FromType8(type3) {
  return IsCyclic(type3) ? FromCyclic(type3.$defs, type3.$ref) : IsIntersect(type3) ? FromIntersect(type3.allOf) : IsUnion(type3) ? FromUnion3(type3.anyOf) : IsTuple(type3) ? FromTuple(type3.items) : IsObject2(type3) ? FromObject4(type3.properties) : {};
}
...
function FromArray4(type3, indexer) {
  ...
  const check4 = Extends({}, normalizedIndexer, Number2());
  const result = exports_result.IsExtendsTrueLike(check4) ? type3 : IsLiteral(indexer) && exports_guard.IsEqual(indexer.const, "length") ? Number2() : Never();
  return result;
}
...
function FromType10(properties4, type3) {
  return IsArray2(type3) ? _Array_(FromType10(properties4, type3.items)) : IsAsyncIterator2(type3) ? AsyncIterator(FromType10(properties4, type3.iteratorItems)) : IsConstructor2(type3) ? Constructor(FromTypes5(properties4, type3.parameters), FromType10(properties4, type3.instanceType)) : IsFunction2(type3) ? _Function_(FromTypes5(properties4, type3.parameters), FromType10(properties4, type3.returnType)) : IsIterator2(type3) ? Iterator(FromType10(properties4, type3.iteratorItems)) : IsPromise(type3) ? _Promise_(FromType10(properties4, type3.item)) : IsTuple(type3) ? Tuple(FromTypes5(properties4, type3.items)) : IsUnion(type3) ? Union(FromTypes5(properties4, type3.anyOf)) : IsIntersect(type3) ? Intersect(FromTypes5(properties4, type3.allOf)) : IsThis(type3) ? _Object_(properties4) : type3;
}
...
function FromObject5(properties4, indexer) {
  const result = IsNumber3(indexer) ? FromIndexerNumber(properties4) : FromIndexer(properties4, indexer);
  return result;
}
...
function IndexElementsWithIndexer(types2, indexer) {
  return types2.reduceRight((result, right, index) => {
    const check4 = Extends({}, Literal(index), indexer);
...
function FromTuple2(types2, indexer) {
  return IsLiteral(indexer) && exports_guard.IsEqual(indexer.const, "length") ? Literal(types2.length) : IsNumber3(indexer) || IsInteger2(indexer) ? FromTupleWithoutIndexer(types2) : FromTupleWithIndexer(types2, indexer);
}

// ../../../../../../../.micro/npm/node_modules/typebox/build/type/engine/indexed/from_type.mjs
function FromType11(type3, indexer) {
  return IsArray2(type3) ? FromArray4(type3.items, indexer) : IsObject2(type3) ? FromObject5(type3.properties, indexer) : IsTuple(type3) ? FromTuple2(type3.items, indexer) : Never();
}
...
function IndexInstantiate(context, state, type3, indexer, options) {
  const instantiatedType = InstantiateType(context, state, type3);
...
function InstanceTypeOperation(type3) {
  return IsConstructor2(type3) ? type3["instanceType"] : Never();
}
...
function InstanceTypeInstantiate(context, state, type3, options = {}) {
  const instantiatedType = InstantiateType(context, state, type3);
...
function FromType12(type3) {
  return IsAny(type3) ? FromAny() : IsArray2(type3) ? FromArray5(type3.items) : IsObject2(type3) ? FromObject6(type3.properties) : IsRecord(type3) ? FromRecord(type3) : IsTuple(type3) ? FromTuple3(type3.items) : Never();
}
...
function KeyOfInstantiate(context, state, type3, options) {
  const instantiatedType = InstantiateType(context, state, type3);
...
function MappedOperation(context, state, identifier2, type3, as, property) {
...
function MappedAction(context, state, identifier2, type3, as, property, options) {
  const result = CanInstantiate([type3]) ? exports_memory.Update(MappedOperation(context, state, identifier2, type3, as, property), {}, options) : MappedDeferred(identifier2, type3, as, property, options);
  return result;
}
function MappedInstantiate(context, state, identifier2, type3, as, property, options) {
  const instantiatedType = InstantiateType(context, state, type3);
...
function NonNullableInstantiate(context, state, type3, options) {
  const instantiatedType = InstantiateType(context, state, type3);
...
function OmitInstantiate(context, state, type3, indexer, options) {
  const instantiatedType = InstantiateType(context, state, type3);
  const instantiatedIndexer = InstantiateType(context, state, indexer);
...
function OptionsInstantiate(context, state, type3, options) {
  const instaniatedType = InstantiateType(context, state, type3);
...
function ParametersOperation(type3) {
  const parameters = IsFunction2(type3) ? type3["parameters"] : [];
  const instantiatedParameters = InstantiateElements({}, { callstack: [] }, parameters);
  const result = Tuple(instantiatedParameters);
  return result;
}
...
function ParametersInstantiate(context, state, type3, options) {
  const instantiatedType = InstantiateType(context, state, type3);
...
function FromType15(type3) {
  return IsCyclic(type3) ? FromCyclic3(type3.$defs, type3.$ref) : IsIntersect(type3) ? FromIntersect3(type3.allOf) : IsUnion(type3) ? FromUnion6(type3.anyOf) : IsObject2(type3) ? FromObject7(type3.properties) : _Object_({});
}
...
function PartialInstantiate(context, state, type3, options) {
  const instantiatedType = InstantiateType(context, state, type3);
...
function PickInstantiate(context, state, type3, indexer, options) {
  const instantiatedType = InstantiateType(context, state, type3);
  const instantiatedIndexer = InstantiateType(context, state, indexer);
...
function FromType17(type3) {
  return IsArray2(type3) ? FromArray6(type3.items) : IsCyclic(type3) ? FromCyclic4(type3.$defs, type3.$ref) : IsIntersect(type3) ? FromIntersect4(type3.allOf) : IsObject2(type3) ? FromObject8(type3.properties) : IsTuple(type3) ? FromTuple4(type3.items) : IsUnion(type3) ? FromUnion7(type3.anyOf) : type3;
}
...
function ReadonlyObjectInstantiate(context, state, type3, options) {
  const instantiatedType = InstantiateType(context, state, type3);
  return ReadonlyObjectAction(instantiatedType, options);
}

// ../../../../../../../.micro/npm/node_modules/typebox/build/type/engine/ref/instantiate.mjs
function RefInstantiate(context, state, type3, ref5) {
  return ref5 in context ? CyclicCheck([ref5], context, context[ref5]) ? type3 : InstantiateType(context, state, context[ref5]) : type3;
}
...
function FromType18(type3) {
  return IsCyclic(type3) ? FromCyclic5(type3.$defs, type3.$ref) : IsIntersect(type3) ? FromIntersect5(type3.allOf) : IsUnion(type3) ? FromUnion8(type3.anyOf) : IsObject2(type3) ? FromObject9(type3.properties) : _Object_({});
}
...
function RequiredInstantiate(context, state, type3, options) {
  const instaniatedType = InstantiateType(context, state, type3);
...
function ReturnTypeOperation(type3) {
  return IsFunction2(type3) ? type3["returnType"] : Never();
}
...
function ReturnTypeInstantiate(context, state, type3, options = {}) {
  const instantiatedType = InstantiateType(context, state, type3);
...
function InstantiateTypes(context, state, types2) {
  return types2.map((type3) => InstantiateType(context, state, type3));
}
...
function InstantiateType(context, state, input) {
  ...
  const instantiated = IsRef2(type3) ? RefInstantiate(context, state, type3, type3.$ref) : IsArray2(type3) ? _Array_(InstantiateType(context, state, type3.items), ArrayOptions(type3)) : IsAsyncIterator2(type3) ? AsyncIterator(InstantiateType(context, state, type3.iteratorItems), AsyncIteratorOptions(type3)) : IsCall(type3) ? CallInstantiate(context, state, type3.target, type3.arguments) : IsConstructor2(type3) ? Constructor(InstantiateTypes(context, state, type3.parameters), InstantiateType(context, state, type3.instanceType), ConstructorOptions(type3)) : IsDeferred(type3) ? InstantiateDeferred(context, state, type3.action, type3.parameters, type3.options) : IsFunction2(type3) ? _Function_(InstantiateTypes(context, state, type3.parameters), InstantiateType(context, state, type3.returnType), FunctionOptions(type3)) : IsIntersect(type3) ? Intersect(InstantiateTypes(context, state, type3.allOf), IntersectOptions(type3)) : IsIterator2(type3) ? Iterator(InstantiateType(context, state, type3.iteratorItems), IteratorOptions(type3)) : IsObject2(type3) ? _Object_(InstantiateProperties(context, state, type3.properties), ObjectOptions(type3)) : IsPromise(type3) ? _Promise_(InstantiateType(context, state, type3.item), PromiseOptions(type3)) : IsRecord(type3) ? RecordFromPattern(RecordPattern(type3), InstantiateType(context, state, RecordValue(type3))) : IsRest(type3) ? Rest(InstantiateType(context, state, type3.items)) : IsTuple(type3) ? Tuple(InstantiateElements(context, state, type3.items), TupleOptions(type3)) : IsUnion(type3) ? Union(InstantiateTypes(context, state, type3.anyOf), UnionOptions(type3)) : type3;
...
function AwaitedInstantiate(context, state, type3, options) {
  const instantiatedType = InstantiateType(context, state, type3);
...
function Evaluate(type3, options = {}) {
...
function EvaluateIntersection(context, type3) {
  ...
  return IsObject2(evaluated) ? Options(evaluated, additionalProperties3) : evaluated;
}
...
function FromTuple5(context, schema5, value) {
  ...
  for (let index = 0;index < length; index++) {
    value[index] = FromType19(context, schema5.items[index], value[index]);
  }
...
function FromType19(context, type3, value) {
  return IsArray2(type3) ? FromArray7(context, type3, value) : IsBase(type3) ? FromBase(context, type3, value) : IsCyclic(type3) ? FromCyclic6(context, type3, value) : IsIntersect(type3) ? FromIntersect6(context, type3, value) : IsObject2(type3) ? FromObject10(context, type3, value) : IsRecord(type3) ? FromRecord2(context, type3, value) : IsRef2(type3) ? FromRef5(context, type3, value) : IsTuple(type3) ? FromTuple5(context, type3, value) : IsUnion(type3) ? FromUnion9(context, type3, value) : value;
}
...
function FromTuple6(context, type3, value) {
  ...
  for (let index = 0;index < Math.min(type3.items.length, value.length); index++) {
    value[index] = FromType20(context, type3.items[index], value[index]);
  }
  return value;
}
...
function FromType20(context, type3, value) {
  return IsArray2(type3) ? FromArray9(context, type3, value) : IsBase(type3) ? FromBase2(context, type3, value) : IsBigInt2(type3) ? FromBigInt6(context, type3, value) : IsBoolean3(type3) ? FromBoolean6(context, type3, value) : IsCyclic(type3) ? FromCyclic7(context, type3, value) : IsEnum2(type3) ? FromEnum2(context, type3, value) : IsInteger2(type3) ? FromInteger(context, type3, value) : IsIntersect(type3) ? FromIntersect7(context, type3, value) : IsLiteral(type3) ? FromLiteral6(context, type3, value) : IsNull2(type3) ? FromNull2(context, type3, value) : IsNumber3(type3) ? FromNumber5(context, type3, value) : IsObject2(type3) ? FromObject12(context, type3, value) : IsRecord(type3) ? FromRecord3(context, type3, value) : IsRef2(type3) ? FromRef6(context, type3, value) : IsString3(type3) ? FromString7(context, type3, value) : IsTemplateLiteral(type3) ? FromTemplateLiteral4(context, type3, value) : IsTuple(type3) ? FromTuple6(context, type3, value) : IsUndefined2(type3) ? FromUndefined2(context, type3, value) : IsUnion(type3) ? FromUnion10(context, type3, value) : IsVoid(type3) ? FromVoid(context, type3, value) : value;
}
...
function FromType21(context, type3, value) {
  ...
  return IsArray2(type3) ? FromArray10(context, type3, defaulted) : IsBase(type3) ? FromBase3(context, type3, defaulted) : IsCyclic(type3) ? FromCyclic8(context, type3, defaulted) : IsIntersect(type3) ? FromIntersect8(context, type3, defaulted) : IsObject2(type3) ? FromObject13(context, type3, defaulted) : IsRecord(type3) ? FromRecord4(context, type3, defaulted) : IsRef2(type3) ? FromRef7(context, type3, defaulted) : IsTuple(type3) ? FromTuple7(context, type3, defaulted) : IsUnion(type3) ? FromUnion11(context, type3, defaulted) : defaulted;
}
...
function FromType22(direction, context, type3, value) {
  return IsArray2(type3) ? FromArray11(direction, context, type3, value) : IsCyclic(type3) ? FromCyclic9(direction, context, type3, value) : IsIntersect(type3) ? FromIntersect9(direction, context, type3, value) : IsObject2(type3) ? FromObject14(direction, context, type3, value) : IsRecord(type3) ? FromRecord5(direction, context, type3, value) : IsRef2(type3) ? FromRef8(direction, context, type3, value) : IsTuple(type3) ? FromTuple8(direction, context, type3, value) : IsUnion(type3) ? FromUnion12(direction, context, type3, value) : Callback(direction, context, type3, value);
}
...
function FromType23(context, type3) {
  return IsArray2(type3) ? FromArray12(context, type3) : IsCyclic(type3) ? FromCyclic10(context, type3) : IsIntersect(type3) ? FromIntersect10(context, type3) : IsObject2(type3) ? FromObject15(context, type3) : IsRecord(type3) ? FromRecord6(context, type3) : IsRef2(type3) ? FromRef9(context, type3) : IsTuple(type3) ? FromTuple9(context, type3) : IsUnion(type3) ? FromUnion13(context, type3) : IsCodec(type3);
}
var visited = new Set;
...
function FromType24(context, type3) {
  return IsDefault(type3) ? FromDefault2(context, type3) : IsArray2(type3) ? FromArray13(context, type3) : IsAsyncIterator2(type3) ? FromAsyncIterator(context, type3) : IsBase(type3) ? FromBase4(context, type3) : IsBigInt2(type3) ? FromBigInt7(context, type3) : IsBoolean3(type3) ? FromBoolean7(context, type3) : IsConstructor2(type3) ? FromConstructor2(context, type3) : IsCyclic(type3) ? FromCyclic11(context, type3) : IsEnum2(type3) ? FromEnum3(context, type3) : IsFunction2(type3) ? FromFunction2(context, type3) : IsInteger2(type3) ? FromInteger2(context, type3) : IsIntersect(type3) ? FromIntersect11(context, type3) : IsIterator2(type3) ? FromIterator(context, type3) : IsLiteral(type3) ? FromLiteral7(context, type3) : IsNever(type3) ? FromNever(context, type3) : IsNull2(type3) ? FromNull3(context, type3) : IsNumber3(type3) ? FromNumber6(context, type3) : IsObject2(type3) ? FromObject16(context, type3) : IsPromise(type3) ? FromPromise(context, type3) : IsRecord(type3) ? FromRecord7(context, type3) : IsRef2(type3) ? FromRef10(context, type3) : IsString3(type3) ? FromString8(context, type3) : IsSymbol2(type3) ? FromSymbol2(context, type3) : IsTemplateLiteral(type3) ? FromTemplateLiteral5(context, type3) : IsTuple(type3) ? FromTuple10(context, type3) : IsUndefined2(type3) ? FromUndefined3(context, type3) : IsUnion(type3) ? FromUnion14(context, type3) : IsVoid(type3) ? FromVoid2(context, type3) : undefined;
}
...
// ../../../../../../../.micro/npm/node_modules/typebox/build/value/equal/equal.mjs
function Equal(left, right) {
  return exports_guard.IsDeepEqual(left, right);
}
// ../../../../../../../.micro/npm/node_modules/typebox/build/value/hash/hash.mjs
function Hash2(value) {
  return exports_hash.Hash(value);
}
// ../../../../../../../.micro/npm/node_modules/typebox/build/value/mutate/error.mjs
class MutateError extends Error {
  constructor(message) {
    super(message);
  }
}
// ../../../../../../../.micro/npm/node_modules/typebox/build/value/mutate/from_array.mjs
...
// ../../../../../../../.micro/npm/node_modules/typebox/build/value/mutate/from_object.mjs
...
// ../../../../../../../.micro/npm/node_modules/typebox/build/value/mutate/from_unknown.mjs
...
// ../../../../../../../.micro/npm/node_modules/typebox/build/value/mutate/from_value.mjs
...
// ../../../../../../../.micro/npm/node_modules/typebox/build/value/mutate/mutate.mjs
...
function Parse(...args) {
  ...
  const checked = Check2(context, type3, value);
  if (checked)
    return value;
...
// ../../../../../../../.micro/npm/node_modules/typebox/build/value/delta/diff.mjs
...
function* FromValue6(path, left, right) {
  return exports_globals.IsTypeArray(left) ? yield* FromTypedArray2(path, left, right) : exports_guard.IsArray(left) ? yield* FromArray15(path, left, right) : exports_guard.IsObject(left) ? yield* FromObject18(path, left, right) : yield* FromUnknown3(path, left, right);
}
...
// ../../../../../../../.micro/npm/node_modules/typebox/build/value/delta/patch.mjs
...
function Patch(current, edits) {
  ...
  const clone3 = Clone2(current);
  for (const edit of edits) {
    switch (edit.type) {
      case "insert": {
        exports_pointer.Set(clone3, edit.path, edit.value);
        break;
      }
      case "update": {
        exports_pointer.Set(clone3, edit.path, edit.value);
      ...
      case "delete": {
        exports_pointer.Delete(clone3, edit.path);
        break;
      }
    }
  }
...
// ../../../../../../../.micro/npm/node_modules/typebox/build/value/repair/error.mjs
...
// ../../../../../../../.micro/npm/node_modules/typebox/build/value/repair/from_array.mjs
...
function FromArray16(context, type3, value) {
  if (Check2(context, type3, value))
    return value;
  const created = exports_guard.IsArray(value) ? value : Create2(context, type3);
  const minimum3 = IsMinItems(type3) && created.length < type3.minItems ? [...created, ...Array.from({ length: type3.minItems - created.length }, () => Create2(context, type3))] : created;
  const maximum3 = IsMaxItems(type3) && minimum3.length > type3.maxItems ? minimum3.slice(0, type3.maxItems) : minimum3;
  const repaired = maximum3.map((value2) => FromType25(context, type3.items, value2));
  if (!IsUniqueItems(type3) || IsUniqueItems(type3) && !exports_guard.IsEqual(type3.uniqueItems, true))
    return repaired;
  const unique = MakeUnique(repaired);
  if (!Check2(context, type3, unique))
    throw new RepairError(context, type3, value, "Failed to repair Array due to uniqueItems constraint");
  return unique;
}

// ../../../../../../../.micro/npm/node_modules/typebox/build/value/repair/from_unknown.mjs
function FromUnknown4(context, type3, value) {
  if (Check2(context, type3, value))
    return value;
  const converted = Convert(context, type3, value);
  if (Check2(context, type3, converted))
    return converted;
  return Create2(context, type3);
}

// ../../../../../../../.micro/npm/node_modules/typebox/build/value/repair/from_base.mjs
function FromBase5(context, type3, value) {
  return FromUnknown4(context, type3, value);
}

// ../../../../../../../.micro/npm/node_modules/typebox/build/value/repair/from_enum.mjs
function FromEnum4(context, type3, value) {
  const union3 = EnumToUnion(type3);
  return FromType25(context, union3, value);
}

// ../../../../../../../.micro/npm/node_modules/typebox/build/value/repair/from_intersect.mjs
function FromIntersect12(context, type3, value) {
  const instantiated = Instantiate(context, type3);
  const evaluated = Evaluate(instantiated);
  return FromType25(context, evaluated, value);
}

// ../../../../../../../.micro/npm/node_modules/typebox/build/value/repair/from_object.mjs
function FromObject19(context, type3, value) {
  if (Check2(context, type3, value))
    return value;
  if (!exports_guard.IsObjectNotArray(value))
    return Create2(context, type3);
  ...
  for (const [key, schema5] of exports_guard.Entries(type3.properties)) {
    if (!required5.has(key) && exports_guard.IsUndefined(value[key]))
      continue;
    result[key] = key in value ? FromType25(context, schema5, value[key]) : Create2(context, schema5);
  }
  const evaluatedKeys = exports_guard.Keys(type3.properties);
  if (IsAdditionalProperties(type3) && exports_guard.IsObject(type3.additionalProperties)) {
    for (const key of exports_guard.Keys(value)) {
      if (evaluatedKeys.includes(key))
        continue;
      result[key] = FromType25(context, type3.additionalProperties, value[key]);
    }
  }
  return result;
}

// ../../../../../../../.micro/npm/node_modules/typebox/build/value/repair/from_record.mjs
function FromRecord8(context, type3, value) {
  if (Check2(context, type3, value))
    return value;
  if (exports_guard.IsNull(value) || !exports_guard.IsObject(value) || exports_guard.IsArray(value))
    return Create2(context, type3);
  const recordKey = new RegExp(RecordPattern(type3));
  const recordValue = RecordValue(type3);
...
// ../../../../../../../.micro/npm/node_modules/typebox/build/value/repair/from_ref.mjs
function FromRef11(context, type3, value) {
  return exports_guard.HasPropertyKey(context, type3.$ref) ? FromType25(context, context[type3.$ref], value) : (() => {
    throw new RepairError(context, type3, value, "Unable to de-reference target type");
  })();
}

// ../../../../../../../.micro/npm/node_modules/typebox/build/value/repair/from_template_literal.mjs
function FromTemplateLiteral6(context, type3, value) {
  const decoded = TemplateLiteralDecode(type3.pattern);
  return FromType25(context, decoded, value);
}

// ../../../../../../../.micro/npm/node_modules/typebox/build/value/repair/from_tuple.mjs
function FromTuple11(context, schema5, value) {
  if (Check2(context, schema5, value))
    return value;
  if (!exports_guard.IsArray(value))
    return Create2(context, schema5);
  return schema5.items.map((schema6, index) => FromType25(context, schema6, value[index]));
}

// ../../../../../../../.micro/npm/node_modules/typebox/build/value/shared/union_score_select.mjs
function Deref(context, type3, value) {
  return IsRef2(type3) ? exports_guard.HasPropertyKey(context, type3.$ref) ? Deref(context, context[type3.$ref], value) : (() => {
    throw new Error("Unable to Deref target");
  })() : type3;
}
function ScoreVariant(context, type3, value) {
  if (!(IsObject2(type3) && exports_guard.IsObject(value)))
    return 0;
  const keys2 = exports_guard.Keys(value);
  const entries = exports_guard.Entries(type3.properties);
  return entries.reduce((result, [key, schema5]) => {
    const literal2 = IsLiteral(schema5) && exports_guard.IsEqual(schema5.const, value[key]) ? 100 : 0;
    const checks = Check2(context, schema5, value[key]) ? 10 : 0;
    const exists = keys2.includes(key) ? 1 : 0;
    return result + (literal2 + checks + exists);
  }, 0);
}
function UnionScoreSelect(context, type3, value) {
  const schemas = type3.anyOf.map((schema5) => Deref(context, schema5, value));
...
// ../../../../../../../.micro/npm/node_modules/typebox/build/value/repair/from_union.mjs
function RepairUnion(context, type3, value) {
  const union3 = Union(Flatten(type3.anyOf));
  const schema5 = UnionScoreSelect(context, union3, value);
  return FromType25(context, schema5, value);
}
function FromUnion15(context, type3, value) {
  if (Check2(context, type3, value))
    return Clone2(value);
  if (IsDefault(type3))
    return Create2(context, type3);
...
// ../../../../../../../.micro/npm/node_modules/typebox/build/value/repair/from_type.mjs
...
function AssertRepairableType(context, type3, value) {
  const unsupported = IsAsyncIterator2(type3) || IsIterator2(type3) || IsConstructor2(type3) || IsFunction2(type3) || IsNever(type3) || IsPromise(type3);
  if (unsupported) {
    throw new RepairError(context, type3, value, "Type is not repairable");
  }
}
function FinalizeRepair(context, type3, repaired) {
  return IsRefine2(type3) ? Check2(context, type3, repaired) ? repaired : Create2(context, type3) : repaired;
}
function FromType25(context, type3, value) {
  if (IsBase(type3)) {
  ...
  const repaired = IsArray2(type3) ? FromArray16(context, type3, value) : IsEnum2(type3) ? FromEnum4(context, type3, value) : IsIntersect(type3) ? FromIntersect12(context, type3, value) : IsObject2(type3) ? FromObject19(context, type3, value) : IsRecord(type3) ? FromRecord8(context, type3, value) : IsRef2(type3) ? FromRef11(context, type3, value) : IsTemplateLiteral(type3) ? FromTemplateLiteral6(context, type3, value) : IsTuple(type3) ? FromTuple11(context, type3, value) : IsUnion(type3) ? FromUnion15(context, type3, value) : FromUnknown4(context, type3, value);
  return FinalizeRepair(context, type3, repaired);
}

// ../../../../../../../.micro/npm/node_modules/typebox/build/value/repair/repair.mjs
function Repair(...args) {
  const [context, type3, value] = exports_arguments.Match(args, {
    3: (context2, type4, value2) => [context2, type4, value2],
    2: (type4, value2) => [{}, type4, value2]
  });
  const repaired = FromType25(context, type3, value);
  Assert(context, type3, repaired);
  return repaired;
}
// ../../../../../../../.micro/npm/node_modules/typebox/build/value/value.mjs
var exports_value = {};
...
// ../../../../../../../.micro/npm/node_modules/typebox/build/value/index.mjs
var value_default = exports_value;
export {
  value_default as default,
  exports_value as Value,
  Update2 as Update,
  UnionScoreSelect,
  UnionPrioritySort,
  Repair,
  exports_pointer as Pointer,
  Pipeline,
  Patch,
  Parser,
