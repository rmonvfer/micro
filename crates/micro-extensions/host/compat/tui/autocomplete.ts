// pi-tui's own autocomplete contract and file/slash-command provider, vendored unchanged:
// `CombinedAutocompleteProvider`'s file matching walks the real filesystem, and its
// `getSuggestions`/`applyCompletion` shape is exactly what an extension implementing
// `ctx.ui.addAutocompleteProvider` writes its own provider against — keeping it byte for
// byte is what keeps that contract single-sourced rather than two definitions that could
// drift apart.
