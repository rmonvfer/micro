// pi-tui's own multi-line editor, vendored unchanged: a `handleInput`/`render` state
// machine built entirely from this compat layer's already-real pieces (keybindings, key
// decoding, the kill ring, the undo stack, word navigation, the autocomplete contract,
// SelectList) — nothing about it reads from or writes to an actual terminal, which is
// what makes `CustomEditor` (pi-subagents' `modal-editor`/`rainbow-editor` examples)
// reachable rather than a load-time failure.
