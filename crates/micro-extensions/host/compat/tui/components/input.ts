// pi-tui's single-line text input, vendored unchanged: a `handleInput`/`render` state
// machine built entirely from this compat layer's already-real pieces (keybindings, key
// decoding, the kill ring, the undo stack, word navigation, grapheme/column utilities) —
// nothing about it reads from or writes to an actual terminal, so nothing about it changes
// running under micro.
