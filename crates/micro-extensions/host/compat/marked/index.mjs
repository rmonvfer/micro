// Real marked 18.0.5 (https://www.npmjs.com/package/marked), MIT licensed, bundled with
// `bun build --target=browser --format=esm` from its published lib/marked.esm.js so it
// carries no external imports of its own — the same reason typebox is vendored this way in
// ../typebox/. Pinned to the exact version pi-tui itself depends on (pi/packages/tui/
// package.json), so pi-tui's own Markdown component gets the identical parser it was built
// and tested against, not merely a same-major one. Regenerate by running the same `bun
// build` command above against node_modules/marked/lib/marked.esm.js after `bun add
// marked@<version>` in a scratch directory.
